...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 14 01:53:16 2020
...
def create_model(captcha_length, captcha_num_symbols, input_shape, model_depth=5, module_size=2):
  ...
  for i, module_length in enumerate([module_size] * model_depth):
      for j in range(module_length):
          ...
          x = keras.layers.Conv2D(64*2**min(i, 3), kernel_size=3, padding='same', kernel_initializer='he_uniform')(x)
          x = keras.layers.BatchNormalization()(x)
          x = keras.layers.Activation('relu')(x)
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            if(len(self.files.keys()) == 0):
                ...
                #print(random.choice(list(self.files.keys())))
            ...
            random_image_label = random_image_label.replace("D","/")
            random_image_label = random_image_label.replace("E", "_")
...
def main():
    ...
    # with tf.device('/device:XLA_CPU:0'):
        ...
        model.compile(loss='categorical_crossentropy',
                      optimizer=keras.optimizers.Adam(5e-5, amsgrad=True),
                      metrics=['accuracy'])
